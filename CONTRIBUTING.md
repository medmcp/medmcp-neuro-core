...
Thanks for your interest in contributing to a medmcp ecosystem package. Please be respectful and follow our [Code of Conduct](CODE_OF_CONDUCT.md).

This file is shared across the medmcp ecosystem packages. If you're reading it inside `medmcp-dicom`, `medmcp-neuro-core`, etc., the same workflow applies — only the repo name differs.
...
The recommended way to work on this package is **inside the dev container**
...
Each medmcp package exposes its functionality as MCP tools — plain typed Python functions that an LLM agent invokes by name. The simplest way to add one is to mirror an existing tool module and its test. When adding one:
...
3. Add unit tests in the matching `tests/test_<group>.py` — call the function directly, no server needed. Skip tests that need an absent external binary with a fixture rather than failing.
...
**Follow the existing conventions.** Match the patterns the current tools already use — for example, a tool that can run on an accelerator takes a `device` argument resolved through the shared helper, so a fall back to CPU is reported rather than silent. When in doubt, copy the closest existing tool.
...
This project uses the [all-contributors](https://allcontributors.org) bot to credit everyone who helps — not just code, but documentation, ideas, bug reports, reviews, and more. The list lives in the README and is tracked in `.all-contributorsrc`.
