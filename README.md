...
| `segment_lesions` | White-matter (MS) lesion segmentation from paired T1w + FLAIR using LST-AI. Run as an external program via the native `lst` venv (preferred) or the `jqmcginnis/lst-ai` Docker image (fallback). Ask the user which device to use (`cpu`/`cuda`); pass `skull_stripped=True` only when both inputs were already brain-extracted. | `t1_path: Path`, `flair_path: Path`, `output_dir: Path \| None = None`, `device: str = "cpu"`, `gpu_id: int = 0`, `skull_stripped: bool = False`, `backend: "native"\|"docker"\|None = None` | `{"lesion_mask_path": "...", "annotated_path": "..."\|null, "output_files": [...], "backend": "...", "device": "...", "_render": "..."}` |
...
| `lesion-segmentation` | Workflow for `segment_lesions`. Covers the paired T1w + FLAIR requirement, the recommended pre-skull-stripping path (run `skull_strip` on both, then `skull_stripped=True`), device choice, native↔docker backend selection, and warping the resulting lesion mask into MNI with `apply_transform` (NearestNeighbor). |
...
| `segment_lesions` | LST-AI lesion-segmentation ensemble | Weights bundled with the LST-AI install (native venv or Docker image); the native `greedy` registration binary is downloaded once to `~/.medmcp_neuro/bin/` if not on PATH | [LST-AI](https://github.com/CompImg/LST-AI) (see upstream repo) |
...
`segment_lesions` runs LST-AI out of process (it is **not** a package dependency — it conflicts with this stack's torch/HD-BET versions). Install it once in its own virtualenv and point `$MEDMCP_LST_AI_BIN` at the `lst` console script, or provide a Docker daemon so the `jqmcginnis/lst-ai` image can be used. It supports CPU (often >1 h per case) and NVIDIA GPU (`device="cuda"`); there is no Apple-Silicon path. Relevant env vars: `MEDMCP_LST_AI_BIN`, `MEDMCP_GREEDY_BIN`, `MEDMCP_LST_AI_IMAGE`, `MEDMCP_LST_AI_BACKEND`.
