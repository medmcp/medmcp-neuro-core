...
- `segment_ms_lesions` tool: white-matter (MS) lesion segmentation from paired T1w + FLAIR via LST-AI, run out of process through the `lst` console script in a dedicated Python 3.11 sidecar venv (`$MEDMCP_LST_AI_BIN`). The `greedy` binary is downloaded on first use. `just lst-ai-setup` builds the sidecar from a commit-pinned lockfile (`lst-ai/`).
- `ms-lesion-segmentation` SKILL.md covering the pre-skull-stripping path (`skull_strip` both inputs → `skull_stripped=True`), device choice, and warping lesion masks into MNI
